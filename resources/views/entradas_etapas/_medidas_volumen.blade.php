...
<div class="row mb-3">
    <div class="col-sm">
        ...
        <input name="ancho" value="{{ old('ancho', $ancho_pivot) }}" id="input-ancho" type="number" step="0.01" min="0.01" class="form-control {{ bootstrap_isInputInvalid('ancho', $errors) }}">
        @include('@.bootstrap.invalid-input-message', ['name' => 'ancho'])
    </div>
    <div class="col-sm">
        <label for="input-altura" class="form-label small">Altura</label>
        <input name="altura" value="{{ old('altura', $altura_pivot) }}" id="input-altura" type="number" step="0.01" min="0.01" class="form-control {{ bootstrap_isInputInvalid('altura', $errors) }}">
        @include('@.bootstrap.invalid-input-message', ['name' => 'altura'])
    </div>
    <div class="col-sm">
        <label for="input-largo" class="form-label small">Largo</label>
        <input name="largo" value="{{ old('largo', $largo_pivot) }}" id="input-largo" type="number" step="0.01" min="0.01" class="form-control {{ bootstrap_isInputInvalid('largo', $errors) }}">
